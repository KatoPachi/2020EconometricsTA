...
```{r variable}
dt$WB <- ifelse(dt$VictimRace == 1 & dt$DefendantRace == 0, 1, 0)
dt$BW <- ifelse(dt$VictimRace == 0 & dt$DefendantRace == 1, 1, 0)
dt$WW <- ifelse(dt$VictimRace == 1 & dt$DefendantRace == 1, 1, 0)
```
...
dt$"(Intercept)" <- 1
X <- as.matrix(dt[,c("(Intercept)", "WB", "BW", "WW")])
...
print("The Robust t-value using White method"); coef(LPM)/se_b
```
